"""
PIPELINE STEP 5: Sequential Transfer Learning
Description:
...
OUTPUT_DIR = os.path.join(BASE_DIR, '..', '..', 'results', 'ltdb_sequential_v1')
...
HP = {
    ...
    "LR_ENCODER": 5e-5,
    "LR_HEAD": 1e-3,
    "MAX_BEATS": 6,  # Must match the loader's max_beats
...
def get_sequential_model():
    ...
    # Replace head: Output is now (MAX_BEATS * NUM_CLASSES)
    # We will reshape this [Batch, 24] -> [Batch, 6, 4] in the training loop
...
def run_transfer_session():
    ...
    # ignore_index=-1 ensures the loss function ignores our padding tokens
    ...
    for epoch in range(HP["EPOCHS"]):
        ...
        for x, y in train_loader:
            ...
            # Forward pass and reshape to [Batch, Beats, Classes]
            logits = model(x).view(-1, HP["MAX_BEATS"], HP["NUM_CLASSES"])
            
            # CrossEntropy expects [Batch, Classes, Sequence_Length]
        ...
        with torch.no_grad():
            for x, y in test_loader:
                ...
                val_loss = criterion(logits.transpose(1, 2), y.to(HP["DEVICE"]))
                total_val_loss += val_loss.item()
        
        avg_train = total_train_loss / len(train_loader)
        avg_val = total_val_loss / len(test_loader)
        print(f"Epoch {epoch+1}/{HP['EPOCHS']} | Train Loss: {avg_train:.4f} | Val Loss: {avg_val:.4f}")
        
        if avg_val < best_loss:
            best_loss = avg_val
            torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, "best_sequential_model.pt"))

    # 4. Final Evaluation (Beat-by-Beat)
    print("\nEvaluating individual beat performance...")
    ...
    with torch.no_grad():
        for x, y in test_loader:
            ...
            # Only collect metrics for non-padding beats
    ...
    # Mapping back to group names for the report
    ...
    print("\n--- BEAT-LEVEL CLASSIFICATION REPORT ---")
    print(report)
